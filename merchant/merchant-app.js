(() => {
  ...
  async function ownStore(user) {
    const rows = await M.request(`stores?select=id,name,description,phone,eta,active,owner_id&owner_id=eq.${encodeURIComponent(user.id)}&limit=1`, { private: true });
    return rows?.[0] || null;
  }
  ...
  async function dashboard() {
    ...
    try {
      const orders = await M.request(`delivery_orders?select=id,status,payable&store_id=eq.${encodeURIComponent(ctx.store.id)}&order=ordered_at.desc&limit=200`, { private: true });
    ...
    } catch (err) { $('#content').innerHTML = M.ui.error('โหลดภาพรวมร้านไม่สำเร็จ', err.message); }
  }
  ...
  async function orders() {
    ...
    if (!ctx) return;
    try {
      const rows = await M.request(`delivery_orders?select=id,customer_name,status,total,payable,ordered_at,delivery_address&store_id=eq.${encodeURIComponent(ctx.store.id)}&order=ordered_at.desc&limit=200`, { private: true });
    ...
    } catch (err) { $('#list').innerHTML = M.ui.error('โหลดออร์เดอร์ไม่สำเร็จ', err.message); }
  }
