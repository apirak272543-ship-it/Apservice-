(() => {
  ...
  async function sendPasswordRecovery(email, redirectTo) { const payload = { email: String(email || '').trim().toLowerCase() }; if (String(redirectTo || '').trim()) payload.redirect_to = redirectTo; return authRequest('recover', { method: 'POST', body: JSON.stringify(payload) }); }
