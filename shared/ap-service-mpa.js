(() => {
  ...
  async function sendPasswordRecovery(email, redirectTo) { const payload = { email: String(email || '').trim().toLowerCase() }; return authRequest(authRequestPath('recover', redirectTo), { method: 'POST', body: JSON.stringify(payload) }); }
