...
| Store status machine | PARTIAL | active, moderation_status, emergency_closed, Merchant RPC และ immutable Store operation event มี | pending/active/temporary/emergency/suspended/archived แยก public visibility, accepting orders, account status, compliance | Merchant owner RPC บังคับ reason/actor สำหรับ emergency; transition matrix เต็มยังขาด | before/after operation event | Customer/Merchant | P2a complete; full state machine P2 |
| Opening hours policy | PARTIAL | weekly schedule 7 วัน, cutoff, emergency close และ **server decision** ว่ารับ order ได้หรือไม่ มีแล้ว | breaks, holiday, special closure และ status snapshot ณ checkout | server validates weekly schedule + catalog/order window | operation audit; order snapshot ยังขาด | Customer/Merchant | P2a complete; extensions P2 |
| GP change history | PARTIAL | current GP, settlement GP snapshot, append-only prior/new/reason history และ Admin action sheet มีแล้ว | approval workflow, scheduled effective date และการผูกอัตรา ณ เวลาสร้าง settlement โดยตรง | Admin-only Edge action; Merchant read-only history | GP history + Admin audit | Admin/Merchant | P2c partial |
...
| Function | Current state | สิ่งที่มีแล้ว | สิ่งที่ยังขาดตาม v2 | Server/Authorization ที่ต้องมี | Audit/Snapshot | APK impact | Priority |
|---|---|---|---|---|---|---|---|
| Rider application / compliance | PARTIAL | vehicle/compliance fields, server document guard ก่อนอนุมัติ, Admin action sheet/audit และ Rider profile status surface มีแล้ว | private document upload UX, retention policy, expiring-document notification และ transition history แยกตาราง | Admin review only; ready status gate server-side | Admin review audit; document submission event ยังขาด | Rider/Admin | P2b partial |
