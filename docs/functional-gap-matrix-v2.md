...
| Function | Current state | สิ่งที่มีแล้ว | สิ่งที่ยังขาดตาม v2 | Server/Authorization ที่ต้องมี | Audit/Snapshot | APK impact | Priority |
|---|---|---|---|---|---|---|---|
| Customer address book | MISSING | profile มี address/location เดียว | multi-address, label, recipient, default, GPS accuracy, delivery note, archive | owner-only CRUD, one default atomically | address change event; **order ต้อง snapshot** | Customer APK | P1 |
| Order address snapshot | PARTIAL | delivery_orders เก็บ delivery_address/location | recipient name/phone, accuracy/source, immutable snapshot object | checkout writes snapshot only, never reads future profile | immutable order snapshot | Customer/Rider/Admin | P1 |
...
| Checkout idempotency | MISSING | POS sale มี idempotency key | idempotency key สำหรับ create delivery order/payment/group checkout | server replay-safe create | request/result linkage | Customer APK | **BLOCKER** |
