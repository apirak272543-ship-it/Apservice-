...
(() => {
  ...
  const CreatorAffiliate = {
        ...
        <section class="admin-section" id="admin-creator-affiliates">
          <div class="panel creator-affiliate-shell">
            ...
            <div class="creator-grid creator-lower-grid">
              ...
              <div class="creator-card creator-howto"><h3>วิธีใช้กับ Creator ท้องถิ่น</h3><p>1. เพิ่ม Creator และบันทึกช่องทางติดต่อ</p><p>2. สร้างแคมเปญพร้อมรหัส เช่น <b>AP-SRI-01</b> และกำหนดค่าคอมมิชชันตามข้อตกลง</p><p>3. ส่งลิงก์ Referral ให้ Creator ใช้ในคลิปหรือโพสต์</p><p>4. ระบบผูกออร์เดอร์กับรหัส แล้วรอให้งานสำเร็จก่อนเสนอค่าคอมมิชชันเข้าคิวอนุมัติ</p><p class="creator-warning">ไม่ควรนำคอนเทนต์ไปใช้โฆษณาต่อจนกว่าจะมีบันทึกสิทธิ์เป็น “อนุญาตใช้งาน”</p></div>
            </div>
            <div class="creator-card creator-table-card"><div class="creator-card-head"><div><h3>รายชื่อ Creator และรหัสแนะนำ</h3><p>กดคัดลอกลิงก์เพื่อส่งให้ Creator ใช้ในคอนเทนต์</p></div></div><div class="table-wrap"><table><thead><tr><th>Creator</th><th>สถานะ</th><th>แคมเปญ / รหัส</th><th>ผลลัพธ์</th><th>จัดการ</th></tr></thead><tbody id="creatorAffiliateRows"></tbody></table></div></div>
    ...
    render() {
      ...
      $('#creatorAffiliateRows').innerHTML = creators.length ? creators.map(creator => {
        ...
        const codeItems = creatorCampaigns.map(campaign => `<div class="creator-code"><b>${esc(campaign.referral_code)}</b><small>${esc(campaign.title)} · ${Number(campaign.commission_rate || 0)}%</small><button class="btn btn-plain btn-small" type="button" onclick="copyCreatorReferralLink('${esc(campaign.referral_code)}','${esc(campaign.landing_path || '/')}')">คัดลอกลิงก์</button></div>`).join('') || '<small>ยังไม่มีรหัส Referral</small>';
    ...
    copyLink(code, path) { const url = `${location.origin}${location.pathname}?ref=${encodeURIComponent(code)}`; navigator.clipboard?.writeText(url).then(() => this.toast(`คัดลอกลิงก์ ${code} แล้ว`, 'success')).catch(() => this.toast(`คัดลอกลิงก์ไม่สำเร็จ โปรดใช้: ${url}`, 'warning')); },
  ...
  window.copyCreatorReferralLink = (code, path) => CreatorAffiliate.copyLink(code, path);
