...
const merchant = fs.readFileSync('merchant/merchant-app.js', 'utf8');
const rider = fs.readFileSync('rider/rider-app.js', 'utf8');

assert.match(media, /version: 'shared-media-v4'/, 'ทุกแอปต้องใช้ Shared Media Service v4 เดียวกัน');
...
assert.match(media, /getMedia\(/, 'ต้องมี resolver สำหรับ public/private media');
...
assert.match(merchant, /STORE_LOGO[\s\S]*STORE_BACKGROUND/, 'Merchant ต้องใช้ profile logo/background กลาง');
assert.match(rider, /mediaType: 'DELIVERY_PROOF', ownerType: 'rider'/, 'Rider proof ต้องเป็น private central media type');
