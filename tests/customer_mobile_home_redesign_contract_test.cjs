...
assert.match(index, /ap-service-mpa\.js\?v=mpa-v3-webview-guard/);
assert.match(index, /data-customer-safe-mode="true"/, 'Home must expose the WebView stability safe mode');
assert.doesNotMatch(index, /customer-facebook-home\.js/, 'Safe mode must isolate the observer-based Facebook home enhancer');
assert.match(index, /customer-mobile-native\.js/, 'Safe mode must keep mobile navigation and core routes available');
