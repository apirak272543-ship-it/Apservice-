...
const checks = [
  ...
  ['customer section heads cancel the admin 220px flex basis', patch.includes('.view:not(#view-admin):not(#view-login):not(#view-register) .section-head>div') && patch.includes('flex:0 1 auto') && patch.includes('height:auto')],
  ['customer views share compact panel and grid gaps', patch.includes('>.grid-2,') && patch.includes('>.panel{padding:14px}')],
];
