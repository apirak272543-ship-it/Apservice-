(() => {
  ...
  async function stores(limit = 12, request = M.request) { return request(`catalog_stores?select=id,name,emoji,description,rating,eta,icon_url,background_url&order=name.asc&limit=${limit}`, { cacheTtlMs: 30_000, cacheKey: `catalog-stores:${limit}` }); }
  async function promotions(request = M.request) { const rows = await request('platform_configs?select=value&key=eq.customer_promotions&limit=1', { cacheTtlMs: 60_000, cacheKey: 'customer-promotions' }); const value = rows?.[0]?.value; const items = Array.isArray(value) ? value : Array.isArray(value?.items) ? value.items : []; return items.filter(item => item?.active !== false && /^https:\/\//i.test(String(item?.image_url || ''))); }
  ...
  async function home() {
    ...
    const [promotionResult, storeResult] = await Promise.allSettled([promotions(scope.request), stores(6, scope.request)]);
  ...
  async function storesPage() {
    ...
    try { $('#storeList').innerHTML = (await stores(12, scope.request)).map(storeCard).join('') || M.ui.empty('ยังไม่มีร้านค้าที่เปิดบริการ'); } catch (err) { if (err.code !== M.network.STALE_RESPONSE) $('#storeList').innerHTML = M.ui.error('โหลดร้านค้าไม่สำเร็จ', err.message); }
  }
