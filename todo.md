...
## Customer Featured Store Carousel

- [x] ตรวจ source ของร้านค้าและกำหนดเกณฑ์คัดเลือกร้านยอดนิยมจากข้อมูลจริงโดยไม่สร้างข้อมูลจำลอง
- [x] แสดงร้านค้าเป็นแถวการ์ดสี่เหลี่ยมเลื่อนแนวนอน แยกตามหมวดหมู่จากข้อมูลจริง โดยคงการค้นหาและตัวกรองเดิม
- [x] จัดอันดับร้านในแต่ละหมวดตามคะแนนรีวิว จำนวนรีวิว จำนวนออร์เดอร์ และสถานะพร้อมให้บริการ เพื่อให้ร้านคุณภาพสูงขึ้นก่อนโดยใช้ข้อมูลจริง
- [x] แสดง Tier 1–5 เฉพาะร้านห้าอันดับสูงสุดของแต่ละหมวด พร้อมตราดาวและเลขระดับที่ลดหลั่นตามอันดับ
- [x] คำนวณ Tier ใหม่จากข้อมูลร้านทุกครั้งที่โหลดหน้า Customer โดยไม่บันทึก Tier หรืออันดับเป็นข้อมูลถาวรในหลังบ้าน
- [x] เล่นเอฟเฟกต์ประกายของ Tier ครั้งแรกที่เห็นและเตือนซ้ำได้ทุก 30 วินาที โดยรองรับการลดการเคลื่อนไหวของอุปกรณ์
- [x] ออกแบบระบบ Recognition สำหรับร้านค้าและไรเดอร์ที่สรุป Tier ผลงานรายสัปดาห์/รายเดือน และสถิติการติดระดับต่อเนื่องจากข้อมูลจริง
- [x] กำหนดแนวทางแจ้งความสำเร็จอัตโนมัติที่บันทึกได้เฉพาะเหตุการณ์ใหม่ ลดการแจ้งซ้ำ และรองรับหน้าต่างคอนเฟตตีตามบทบาท
- [x] สร้างงานประมวลผล Recognition อัตโนมัติรายวัน พร้อมปิดผลรายสัปดาห์และรายเดือน เพื่อบันทึกเฉพาะความสำเร็จใหม่ที่ยืนยันได้
- [x] กำหนดการป้องกันการแจ้งซ้ำและการส่งเหตุการณ์ Recognition ให้เฉพาะเจ้าของร้านค้าหรือไรเดอร์ที่เกี่ยวข้อง
- [x] กำหนดข้อมูล Recognition ที่หน้าโปรไฟล์ของร้านค้าและไรเดอร์จะแสดง เช่น Tier ปัจจุบัน อันดับพิเศษ ลำดับติดระดับต่อเนื่อง คะแนนรีวิว และผลการทำงานที่มีสิทธิ์เห็น
- [x] เพิ่มป้ายผู้ใช้งานคุณภาพสูงแบบอันดับ Top 50 สำหรับผู้ที่ไม่ติด Tier 1–3 โดยคำนวณจากข้อมูลจริงและไม่เปิดเผยข้อมูลส่วนบุคคลที่ไม่จำเป็น
- [x] เพิ่มการ์ด Recognition ในโปรไฟล์ร้านค้าและไรเดอร์ พร้อมข้อมูลส่วนตัวที่ RLS อนุญาตให้เจ้าของเห็นเท่านั้น
- [x] เพิ่ม popup ความสำเร็จใหม่พร้อมคอนเฟตตีและประกายดาวแบบหนึ่งครั้งต่อ session รวม reduced-motion fallback
- [x] เพิ่ม regression contract และ behavioral test สำหรับ popup เหตุการณ์ใหม่ การทำเครื่องหมายเห็นแล้ว และ Android WebView parity
- [ ] สร้างส่วนร้านค้ายอดนิยมแบบการ์ดสี่เหลี่ยมจัตุรัสที่เลื่อนซ้าย–ขวาและรองรับการสัมผัส
- [ ] จำกัดรายการเด่นอัตโนมัติไว้ไม่เกิน 5 ร้าน พร้อม empty state เมื่อไม่มีร้านที่เข้าเกณฑ์
- [x] คงการกดทั้งการ์ดเพื่อเปิดเมนูร้าน และเพิ่มการเข้าถึงด้วยคีย์บอร์ด
- [x] เพิ่ม regression contract และทดสอบการเลื่อนแนวนอน การเลือกข้อมูลจริง และ navigation
- [ ] ตรวจ diff บันทึก commit และ push การเปลี่ยนแปลงหลังอัปเดตจากงานร่วม

## Admin-Managed Featured Store Promotions

- [ ] ตรวจโครงสร้าง Admin และ Android Admin shell สำหรับเมนูจัดการพื้นที่โปรโมตร้านค้า
- [ ] ออกแบบและสร้างข้อมูลตั้งค่าโหมดอัตโนมัติหรือโฆษณา พร้อมร้านเป้าหมาย ลำดับ และช่วงเวลาเผยแพร่
- [ ] สร้างหน้าจอ Admin สำหรับเลือกโหมด กำหนดร้าน ตั้งวันเวลา และจัดลำดับรายการโปรโมต
- [ ] ปรับ Customer ให้ดึงรายการที่ Admin กำหนดและถอยกลับสู่การคัดเลือกร้านอัตโนมัติเมื่อไม่มีรายการใช้งาน
- [ ] ปรับ Android Admin shell ให้รับรู้และนำทางสอดคล้องกับการจัดการพื้นที่โปรโมตร้านค้า
- [ ] ทดสอบสิทธิ์ Admin ช่วงเวลา รายการหมดอายุ การคัดเลือกจากข้อมูลจริง และการแสดงผลแบบ empty state
...
- [x] ตรวจและขยายการแก้ไขร้านให้แก้ข้อมูลร้านเป็นรายการ ทั้งข้อมูลติดต่อ เจ้าของ ที่อยู่ พิกัด เอกสาร สื่อ และการตั้งค่าการปฏิบัติงาน
- [x] ตรวจและขยายฟอร์มเพิ่มร้าน/บัญชี Merchant ให้เก็บข้อมูลจำเป็นของร้านอย่างครบถ้วนโดยไม่เขียนทับฟิลด์ที่ไม่เกี่ยวข้อง
- [x] ออกแบบและพัฒนา workflow คำขอถอนเงินที่แสดงข้อมูลผู้รับ บัญชี/QR ที่ลงทะเบียน หลักฐานการโอน และ audit trail ก่อนบันทึกจ่าย
- [x] ตรวจและขยายการจัดการบัญชีรายบุคคล ทั้งบทบาท สิทธิ์ การระงับ ข้อมูลติดต่อ และค่าทางการเงินตาม server authorization
- [ ] ลดหน้า Login ทุกแอปให้เหลือเฉพาะข้อมูลและ controls ที่จำเป็น ไม่มีข้อความเทคนิคหรือคำอธิบายระบบ
- [x] ลดหน้า Login ของ Admin ให้เหลือเฉพาะ form และ controls ที่จำเป็น ไม่มีข้อความเทคนิคหรือคำอธิบายระบบ
- [ ] ตรวจและลดข้อความระบบในหน้า Login ของ Customer, Merchant, Rider และ AP Retail POS
- [x] ทำ capability audit เทียบ legacy, schema และ workflow จริง เพื่อเพิ่มฟังก์ชัน Admin Control Plane ที่จำเป็นแต่ยังขาดโดยไม่รอคำสั่งรายจุด
- [x] เพิ่ม regression tests, ตรวจผลกระทบ 5 Web Apps/4 WebView APK และเผยแพร่ Admin Control Plane completeness
