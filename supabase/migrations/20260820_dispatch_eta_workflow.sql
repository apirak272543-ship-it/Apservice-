-- Wave P1: complete Admin dispatch/assignment and customer ETA visibility.
-- Existing order status transitions remain unchanged; this adds dispatch metadata only.
...
ALTER TABLE public.delivery_orders
  DROP CONSTRAINT IF EXISTS delivery_orders_dispatch_status_check;
ALTER TABLE public.delivery_orders
  ADD CONSTRAINT delivery_orders_dispatch_status_check
  CHECK (dispatch_status IN ('unassigned', 'assigned', 'en_route', 'arrived_pickup', 'picked_up', 'delivering', 'delivered', 'exception'));

UPDATE public.delivery_orders
SET
  dispatch_status = CASE WHEN rider_id IS NULL THEN 'unassigned' ELSE 'assigned' END,
  assigned_at = CASE WHEN rider_id IS NULL THEN NULL ELSE COALESCE(accepted_at, updated_at, now()) END,
  dispatch_updated_at = COALESCE(updated_at, now())
WHERE dispatch_status = 'unassigned';

CREATE INDEX IF NOT EXISTS delivery_orders_dispatch_queue_idx
  ON public.delivery_orders(dispatch_status, estimated_arrival_at, updated_at DESC);
CREATE INDEX IF NOT EXISTS delivery_orders_rider_dispatch_idx
  ON public.delivery_orders(rider_id, dispatch_status, updated_at DESC);

CREATE TABLE IF NOT EXISTS public.delivery_dispatch_events (
  id bigint GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  order_id text NOT NULL REFERENCES public.delivery_orders(id) ON DELETE CASCADE,
  rider_id text,
  actor_id uuid,
  actor_role text NOT NULL CHECK (actor_role IN ('admin', 'rider', 'system')),
  event_type text NOT NULL CHECK (event_type IN ('assigned', 'unassigned', 'eta_set', 'eta_cleared', 'status_updated', 'exception')),
  dispatch_status text NOT NULL CHECK (dispatch_status IN ('unassigned', 'assigned', 'en_route', 'arrived_pickup', 'picked_up', 'delivering', 'delivered', 'exception')),
  estimated_arrival_at timestamptz,
  note text,
  idempotency_key text NOT NULL,
  created_at timestamptz NOT NULL DEFAULT now(),
  CONSTRAINT delivery_dispatch_events_idempotency_unique UNIQUE (order_id, idempotency_key)
);

CREATE INDEX IF NOT EXISTS delivery_dispatch_events_order_idx
  ON public.delivery_dispatch_events(order_id, created_at ASC);
CREATE INDEX IF NOT EXISTS delivery_dispatch_events_rider_idx
  ON public.delivery_dispatch_events(rider_id, created_at DESC);

ALTER TABLE public.delivery_dispatch_events ENABLE ROW LEVEL SECURITY;
DROP POLICY IF EXISTS "dispatch_events_admin_all" ON public.delivery_dispatch_events;
CREATE POLICY "dispatch_events_admin_all" ON public.delivery_dispatch_events
  FOR ALL TO authenticated
  USING (private.has_role('admin'))
  WITH CHECK (private.has_role('admin'));

DROP POLICY IF EXISTS "dispatch_events_customer_read" ON public.delivery_dispatch_events;
CREATE POLICY "dispatch_events_customer_read" ON public.delivery_dispatch_events
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1 FROM public.delivery_orders o
    WHERE o.id = delivery_dispatch_events.order_id
      AND o.customer_id = auth.uid()
  ));

DROP POLICY IF EXISTS "dispatch_events_rider_read" ON public.delivery_dispatch_events;
CREATE POLICY "dispatch_events_rider_read" ON public.delivery_dispatch_events
  FOR SELECT TO authenticated
  USING (EXISTS (
    SELECT 1
...
GRANT SELECT ON public.delivery_dispatch_events TO authenticated;
GRANT SELECT, UPDATE ON public.delivery_orders TO authenticated;
NOTIFY pgrst, 'reload schema';

DROP FUNCTION IF EXISTS public.admin_update_order_dispatch(text,text,text,integer,boolean,text,text,text);
CREATE FUNCTION public.admin_update_order_dispatch(
  p_order_id text,
